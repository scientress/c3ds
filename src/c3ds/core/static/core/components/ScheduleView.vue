...
<template>
  <div class="grid grid-cols-schedule text-3xl gap-2 text-highlight">
    <template v-for="talk in next_talks" :key="talk.guid">
      <div class="font-numbers font-semibold text-4xl">{{ talk.start }}</div>
      ...
      <div :style='{background: `linear-gradient(90deg, #29114C ${talk.percent_completed}%, rgba(0,0,0,0) ${talk.percent_completed}%)`}'>
        <h2 class="font-headline font-semibold text-4xl">{{ talk.title }}</h2>
